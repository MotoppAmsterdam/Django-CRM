...
class ContactsListView(APIView, LimitOffsetPagination):
    ...
    def get_contact_ids_by_category(self, category):
        """
        Get contact IDs that belong to the specified category.
        """
        ...
        if category == "Lead":
            return Lead.objects.filter(
                Q(status__in=["in process", "assigned"]) & Q(contacts__isnull=False)
            ...
            return Opportunity.objects.filter(stage="CLOSED WON").values_list("contacts__id", flat=True)

        elif category == "Loyal Customer":
            return Opportunity.objects.filter(stage="CLOSED WON") \
    ...
    def get_contact_categories(self, contact_id):
        ...
        try:
            ...
            if closed_won_count > 1:
                categories["Loyal Customer"] = [
                    opportunity.id for opportunity in opportunities.filter(stage="CLOSED WON")]
            ...
            for opportunity in opportunities:
                if opportunity.stage in [
                    ...
                    categories.setdefault(
                        "Customer", []).append(opportunity.id)
...
class ContactDetailView(APIView):
    ...
    def put(self, request, pk, format=None):
        ...
        address_serializer = BillingAddressSerializer(
            data=data, instance=address_obj)
        data = {}
        if not contact_serializer.is_valid():
            data["contact_errors"] = contact_serializer.errors
        if not address_serializer.is_valid():
